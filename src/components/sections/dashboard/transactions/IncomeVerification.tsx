...
const getSourceType = (t: Transaction): SourceType => {
  ...
  if (t.category === 'other income' && /\bbenefit\b|\bpension\b|\buniversal credit\b/.test(desc)) return 'Benefits';
  if (/\brent\b|\brental\b/.test(desc) && (t.money_in || 0) > 0) return 'Rental';
  if (/\binvoice\b|\bupwork\b|\bfreelance\b|\bcontract\b|\bconsult/.test(desc)) return 'Self-employment';
...
const IncomeVerification = ({ transactionData }: IncomeVerificationProps) => {
    ...
    for (const { type, payer, txs } of groups.values()) {
      ...
      if (months.length >= 3) {
        ...
        for (let i = 1; i < months.length; i++) {
          ...
          if (gap > 1) { monthly = false; break; }
        }
      }
      const flags: string[] = [];
      for (let i = 1; i < values.length; i++) {
        if (values[i - 1] > 0 && values[i] >= 2 * values[i - 1]) {
          flags.push(`${type} increased by ${Math.round((values[i] / values[i - 1]) * 100)}% in ${months[i]}`);
        }
      }
      ...
      results.push({ sourceType: type, payer, frequency: monthly ? 'monthly' : 'irregular', averageAmount: avg, flags });
    }

    const order: SourceType[] = ['Salary', 'Self-employment', 'Benefits', 'Rental', 'Other recurring income'];
  ...
  const kpis = useMemo(() => {
    ...
    const suddenFlags = sources.reduce((acc, s) => acc + s.flags.filter((f) => /increased/i.test(f)).length, 0);
      ...
      <Box sx={{ flex: 1, border: '1px solid', borderColor: 'grey.200', backgroundColor: 'common.white', borderRadius: 2, p: { xs: 2, sm: 3 } }}>
        <Stack spacing={2}>
          ...
          <Typography variant="caption" color="text.secondary" sx={{ mb: 1 }}>
            Checks Performed: Identify main income sources • Confirm regularity • Flag sudden increases/irregularities • Compare average monthly income
          </Typography>
          ...
          <Stack spacing={2}>
            <Grid container spacing={2}>
              {sources.map((s, idx) => (
                <Grid item xs={12} md={sources.length === 1 ? 12 : 6} key={`${s.sourceType}-${s.payer}-${idx}`}>
                  <Box sx={{ border: '1px dashed', borderColor: 'grey.200', borderRadius: 1, p: { xs: 2, sm: 2.5 }, backgroundColor: 'grey.50', height: '100%' }}>
                    <Stack spacing={1.25} sx={{ height: '100%' }}>
                      <Stack direction="row" alignItems="center" justifyContent="space-between">
                        <Typography variant="body2" sx={{ fontWeight: 700 }}>
                          {s.sourceType}
                        </Typography>
                        <Chip label={s.frequency === 'monthly' ? 'Monthly' : 'Irregular'} color={s.frequency === 'monthly' ? 'success' : 'warning'} size="small" />
                      </Stack>
                      <Stack spacing={0.75}>
                        <Typography variant="caption" color="text.secondary">Employer / Payer</Typography>
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>{s.payer}</Typography>
                      </Stack>
                      <Stack spacing={0.75}>
                        <Typography variant="caption" color="text.secondary">Average amount</Typography>
                        <Typography variant="body2" sx={{ fontWeight: 700 }}>{fmt(s.averageAmount)}</Typography>
                      </Stack>
                      {s.flags.length > 0 && (
            ...
            {sources.length === 0 && (
              <Typography variant="body2" color="text.secondary">No income sources detected.</Typography>
            )}
          </Stack>
        </Stack>
      </Box>
